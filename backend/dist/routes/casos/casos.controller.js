...
exports.CasosCrontroller = void 0;
const db_1 = __importDefault(require("../../db"));
const casos_sql_1 = require("./casos.sql");
const logger_1 = require("../../services/logger");
const casos_service_1 = require("./casos.service");
const casos_middleware_1 = require("./casos.middleware");
class CasosCrontroller {
    static async create(req, res) {
        // necessidade de definir mais os erros de rotas?
        try {
            const novoCaso = casos_service_1.CasosService.createCaso(req.body, req.user);
    ...
    static async list(req, res) {
        ...
        try {
            let query = casos_sql_1.CASOS_SQL.SELECT_BASE;
            const params = [];
            const whereClauses = [];
            ...
            if (searchTerm) {
                ...
                whereClauses.push(casos_sql_1.CASOS_SQL.CLEAN(`
              (nome ILIKE ${p1} OR
               "tecRef" ILIKE ${p2} OR
    ...
              `));
            ...
            console.log("DEBUG: FINAL QUERY:", casos_sql_1.CASOS_SQL.CLEAN(query));
            console.log("DEBUG: FINAL PARAMS:", params);
            // Execução
            const result = await db_1.default.query(casos_sql_1.CASOS_SQL.CLEAN(query), params);
            const dadosProcessados = (0, casos_middleware_1.anonimizarDadosSeNecessario)(user, result.rows);
    ...
    static async update(req, res) {
        ...
        try {
            const resultAtual = await db_1.default.query(casos_sql_1.CASOS_SQL.CLEAN(casos_sql_1.CASOS_SQL.SELECT_BY_ID), [id]);
            ...
            await db_1.default.query(casos_sql_1.CASOS_SQL.CLEAN(casos_sql_1.CASOS_SQL.UPDATE), [dataCad, tecRef, nome, JSON.stringify(dadosMesclados), id]);
    ...
    static async patch(req, res) {
        ...
        try {
            const result = await db_1.default.query(casos_sql_1.CASOS_SQL.CLEAN(casos_sql_1.CASOS_SQL.UPDATE_STATUS), [status, id]);
    ...
    static async delete(req, res) {
        ...
        const { id: userId, username } = req.user;
        try {
            const result = await db_1.default.query(casos_sql_1.CASOS_SQL.CLEAN(casos_sql_1.CASOS_SQL.DELETE), [id]);
            if (result.rowCount === 0)
                return res.status(404).json({ message: 'Caso não encontrado.' });
    ...
    static async getCaso(req, res) {
        ...
        const checkQuery = casos_sql_1.CASOS_SQL.CLEAN(`SELECT * FROM casos WHERE id = $1 AND ${finalUnitWhere}`);
        try {
            ...
            const demandasQuery = casos_sql_1.CASOS_SQL.CLEAN(`
    ...
            `);
            ...
            const dadosProcessados = (0, casos_middleware_1.anonimizarDadosSeNecessario)(user, casoCompleto);
            res.json(dadosProcessados);
        }
        catch (err) {
    ...
    static async getEncaminhamentos(req, res) {
        ...
        const checkQuery = casos_sql_1.CASOS_SQL.CLEAN(`
    ...
    static async getFast(req, res) {
        ...
        try {
            ...
            const searchClause = casos_sql_1.CASOS_SQL.CLEAN(`
                (nome ILIKE ${p1} OR
            ...
            const query = casos_sql_1.CASOS_SQL.CLEAN(`
...
exports.CasosCrontroller = CasosCrontroller;
