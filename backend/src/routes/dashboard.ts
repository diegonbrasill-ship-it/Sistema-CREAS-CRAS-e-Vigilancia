...
            pool.query(cleanSqlString(`SELECT COUNT(id) AS total FROM casos WHERE dados_completos->>'recebeBPC' IN ('Idoso', 'PCD') ${andClause}`), params),
...
            
            // 9 - 12 (Principais: CORREÇÃO GROUP BY)
            // 📌 FIX: Adiciona a coluna agrupada na clausula GROUP BY
            pool.query(cleanSqlString(`SELECT dados_completos->>'tipoMoradia' AS name FROM casos ${whereClause} GROUP BY dados_completos->>'tipoMoradia' ORDER BY COUNT(*) DESC LIMIT 1`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'escolaridade' AS name FROM casos ${whereClause} GROUP BY dados_completos->>'escolaridade' ORDER BY COUNT(*) DESC LIMIT 1`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'tipoViolencia' AS name FROM casos ${whereClause} GROUP BY dados_completos->>'tipoViolencia' ORDER BY COUNT(*) DESC LIMIT 1`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'localOcorrencia' AS name FROM casos ${whereClause} GROUP BY dados_completos->>'localOcorrencia' ORDER BY COUNT(*) DESC LIMIT 1`), params),
            
            // 13 a 19 - Gráficos (CORREÇÃO GROUP BY)
            pool.query(cleanSqlString(`SELECT dados_completos->>'bairro' as name, COUNT(*) as value FROM casos ${whereClause} GROUP BY dados_completos->>'bairro' ORDER BY value DESC LIMIT 5`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'tipoViolencia' as name, COUNT(*) as value FROM casos ${whereClause} GROUP BY dados_completos->>'tipoViolencia' ORDER BY value DESC`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'encaminhamentoDetalhe' as name, COUNT(*) as value FROM casos ${whereClause} GROUP BY dados_completos->>'encaminhamentoDetalhe' ORDER BY value DESC LIMIT 5`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'sexo' as name, COUNT(*) as value FROM casos ${whereClause} GROUP BY dados_completos->>'sexo' ORDER BY value DESC`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'canalDenuncia' as name, COUNT(*) as value FROM casos ${whereClause} GROUP BY dados_completos->>'canalDenuncia' ORDER BY value DESC`), params),
            pool.query(cleanSqlString(`SELECT dados_completos->>'corEtnia' as name, COUNT(*) as value FROM casos ${whereClause} GROUP BY dados_completos->>'corEtnia' ORDER BY value DESC`), params),
            pool.query(cleanSqlString(`SELECT CASE WHEN (dados_completos->>'idade')::integer BETWEEN 0 AND 11 THEN 'Criança (0-11)' WHEN (dados_completos->>'idade')::integer BETWEEN 12 AND 17 THEN 'Adolescente (12-17)' WHEN (dados_completos->>'idade')::integer BETWEEN 18 AND 29 THEN 'Jovem (18-29)' WHEN (dados_completos->>'idade')::integer BETWEEN 30 AND 59 THEN 'Adulto (30-59)' WHEN (dados_completos->>'idade')::integer >= 60 THEN 'Idoso (60+)' ELSE 'Não informado' END as name, COUNT(*) as value FROM casos ${whereClause} GROUP BY name ORDER BY value DESC`), params),
            
            // 20, 21, 22 - Opções para os Filtros (CORREÇÃO GROUP BY)
...
            pool.query(cleanSqlString(`SELECT DISTINCT dados_completos->>'bairro' AS bairro FROM casos ${whereClause} AND dados_completos->>'bairro' IS NOT NULL AND dados_completos->>'bairro' <> '' GROUP BY bairro ORDER BY bairro ASC`), params)
        ];
