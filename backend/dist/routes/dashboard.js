 ...
 */
const buildFullWhereClauseContent = (filters, accessFilter, startParamIndex) => {
    const whereClauses = [];
    let params = [];
...
router.get("/", async (req, res) => {
    try {
        ...
        const { mes, tecRef, bairro } = req.query;
        ...
        const getGroupedFieldName = (jsonbKey) => {
            const jsonbField = `dados_completos->>'${jsonbKey}'`;
        ...
        const queries = [
            ...
            db_1.default.query(cleanSqlString(`SELECT
...
                FROM casos ${whereClause}`), params),
            ...
            db_1.default.query(cleanSqlString(`SELECT dados_completos->>'tipoMoradia' AS name FROM casos ${whereClause} AND dados_completos->>'tipoMoradia' IS NOT NULL AND TRIM(dados_completos->>'tipoMoradia') <> '' GROUP BY dados_completos->>'tipoMoradia' ORDER BY COUNT(*) DESC LIMIT 1`), params),
            ...
            db_1.default.query(cleanSqlString(`SELECT DISTINCT dados_completos->>'bairro' AS bairro FROM casos ${whereClause} AND dados_completos->>'bairro' IS NOT NULL AND TRIM(dados_completos->>'bairro') <> '' GROUP BY bairro ORDER BY bairro ASC`), params)
        ];
        ...
        const responsePayload = {
            dados: {
                ...
                graficos: {
                    casosPorBairro: results[13].rows.map((r) => ({ ...r, value: parseInt(r.value, 10) })),
                    tiposViolacao: results[14].rows.map((r) => ({ ...r, value: parseInt(r.value, 10) })),
                    encaminhamentosTop5: results[15].rows.map((r) => ({ ...r, value: parseInt(r.value, 10) })),
                    casosPorSexo: results[16].rows.map((r) => ({ ...r, value: parseInt(r.value, 10) })),
                    canalDenuncia: results[17].rows.map((r) => ({ ...r, value: parseInt(r.value, 10) })),
                    casosPorCor: results[18].rows.map((r) => ({ ...r, value: parseInt(r.value, 10) })),
                    casosPorFaixaEtaria: results[19].rows.map((r) => ({ ...r, value: parseInt(r.value, 10) }))
                }
            },
            opcoesFiltro: {
                meses: results[20].rows.map((r) => r.mes),
                tecnicos: results[21].rows.map((r) => r.tecRef),
                bairros: results[22].rows.map((r) => r.bairro),
            }
        };
