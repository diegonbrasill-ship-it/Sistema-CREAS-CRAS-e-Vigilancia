...
// Tipos
// ✅ CORREÇÃO: Adicionamos nome_completo para uso na designação
interface User { id: number; username: string; role: string; nome_completo: string; } 
...
  // ⭐️ LÓGICA CORRIGIDA: Efeito da "Designação Inteligente"
    useEffect(() => {
    if (selectedCaso) {
      // O selectedCaso.tecRef vem como "Nome Completo - Cargo". 
      // Tentamos encontrar o usuário cujo nome completo está DENTRO do tecRef do caso
      const tecnicoDoCaso = allUsers.find(u => 
        u.nome_completo && selectedCaso.tecRef.includes(u.nome_completo) 
      ); 

      if (tecnicoDoCaso) {
        setFormData(prev => ({ ...prev, tecnico_designado_id: String(tecnicoDoCaso.id) }));
      } else {
        // ✅ Estabilização: Se não encontrar o técnico (o que causava a tela branca), 
        // limpamos a designação para forçar a seleção manual.
        setFormData(prev => ({ ...prev, tecnico_designado_id: '' })); 
      }
    }
...
    setCasoSearchTerm(caso.nome); // Preenche o campo de busca com o nome (ou [DADO SIGILOSO] se for o caso)
...
                </SelectTrigger>
                <SelectContent>
                  {/* ⭐️ CORREÇÃO: Filtra perfis elegíveis e usa nome_completo */}
                  {allUsers.filter(u => 
                      u.role.includes('tecnico') || 
                      u.role.includes('coordenador') || 
                      u.role.includes('vigilancia')
                    ).map(user => (
                    <SelectItem key={user.id} value={String(user.id)}>{user.nome_completo || user.username} ({user.role})</SelectItem>
                  ))}
                </SelectContent>
              </Select>
